...
function hideFallbackWarning() {
  if (banner) {
    if (bannerCounter > 0) bannerCounter--;
    else {
...
function prepare(userParams, defaults) {
  ...
  // Fix: Handle single object parameter
  if (
    userParams &&
  ...
  for (var name of Object.keys(params)) {
    if (typeof params[name] === "number") params[name] = float(params[name]);
...
brain.normal = TSLFn((params) => {
  ...
  var dTime = mx_noise_float(pos.mul(params.wave.mul(5)))
    .add(1)
    .div(2)
    .mul(6.28);
  return vec3(
    dx,
    dy,
    time.mul(params.speed).add(dTime).sin().add(n, 1)
  ).normalize();
}, defaults$M);
...
var circles = TSLFn((params) => {
  ...
  for (var n = 0; n <= 10; n++) {
    k.addAssign(
      sin(x.mul(2 ** n).sub((Math.PI * n) / 2)).mul((-n * (n + 1)) / 2)
    );
  }
...
var _clouds = Fn((params) => {
  ...
  const k = add(
    ...
    noised(pos, scale, 2, params.seed).mul(0.8),
    noised(pos, scale, 6, params.seed).mul(0.1),
...
var concrete = TSLFn((params) => {
  ...
  var posV = surfacePos$7(
    position.add(bitangent),
    normal,
    params.bump,
    density,
    seed
...
var dalmatianSpots = TSLFn((params) => {
  ...
  Loop(count, () => {
    k.mulAssign(
      mx_noise_float(pos).abs().pow(d).mul(100).sub(50).clamp(0, 1).oneMinus()
    );
    pos.assign(pos.mul(1.01));
    k.mulAssign(
      mx_noise_float(pos.yzx)
        .abs()
        .pow(d)
        .mul(100)
        .sub(50)
...
var darthMaul = TSLFn((params) => {
  ...
  var c = select(
    position.x.greaterThan(
      mx_noise_float(position.mul(2.3)).abs().mul(0.5).add(0.02)
    ),
    1,
    0
  );
...
var gasGiant = TSLFn((params) => {
  ...
  // turbulence strength
  var turbulence = params.turbulence
    .mul(
  ...
  var spot = mx_noise_float(pos.div(4))
    .add(1)
    .div(2)
    .pow(10)
    .mul(10)
  ...
  pos.addAssign(
    vec3(
      mx_noise_float(pos),
      mx_noise_float(pos.yxz),
      mx_noise_float(pos.yzx)
    ).mul(turbulence.mul(spot.mul(2).exp()))
  );
  ...
  var color = hsl(
    HSL.x.add(mx_noise_float(pos.mul(vec3(0, scale, 0))).div(4)),
    HSL.y,
    HSL.z
  ).toVar();
...
var grid = TSLFn((params) => {
  ...
  var uv = select(
      params.flat,
      screenUV,
      equirectUV(positionGeometry.normalize())
    ).toVar(),
  ...
  var k = oneMinus(
    smoothstep(sub(treshold, 0.002), add(treshold, 0.002), angle)
  );
...
var isolines = TSLFn((params) => {
  ...
  k = smoothstep(
    sub(params.thinness, params.blur),
    add(params.thinness, params.blur),
    k
  );
...
var karstRock = TSLFn((params) => {
  ...
  var k = mx_noise_float(pos2)
    .div(mx_noise_float(pos2.mul(1.01)))
    .clamp(0, 2)
    .toVar();
...
var neonLights = TSLFn((params) => {
  ...
  color.addAssign(
    select(params.mode.equal(0), neon, neon.negate()).mul(thinness)
  );
...
var planet = TSLFn((params) => {
  ...
  Loop(params.iterations.add(10), () => {
    k.addAssign(
      mul(power, mx_noise_float(positionGeometry.mul(scale).add(params.seed)))
    );
    sum.addAssign(power);
    scale.mulAssign(1.5);
    power.mulAssign(0.8);
    ...
    .Else(() => {
      ...
      color.assign(
        mix(
          ...
          params.colorSnow,
          smoothstep(
            mix(levelSnow, levelMountain, params.balanceSnow.pow(0.5)),
            levelSnow,
            k
          )
        )
...
var processedWood = TSLFn((params) => {
  ...
  k = k.mul(
    mx_noise_float(pos.mul(vec3(25, 1, 1)))
      .add(-1)
      .mul(0.2)
  );
...
var protozoa = TSLFn((params) => {
  ...
  return mix(
    params.background,
    mix(params.color, params.subcolor, n2.mul(0.1)),
    n1
  );
}, defaults$q);
...
var surfacePos$5 = Fn(([pos, normal, bump, curvature]) => {
  var k1 = mx_worley_noise_float(pos.add(mx_noise_float(pos).mul(curvature)))
    .add(0.8)
    .pow(5)
    .toVar();
...
var roughClay = TSLFn((params) => {
  ...
  var posU = surfacePos$5(
    position.add(tangent),
    normal,
    bump,
    params.curvature
  );
  var posV = surfacePos$5(
    position.add(bitangent),
    normal,
    bump,
    params.curvature
...
var runnyEggs = TSLFn((params) => {
  ...
  return mix(
    ...
    mix(params.colorWhite, params.colorYolk, yolks),
    whites
  );
}, defaults$n);
...
var _rust = Fn((params) => {
  ...
  var amount = params.amount
    .mul(mx_noise_float(pos.mul(params.amount.div(2).add(4))).add(4))
    .toVar();
...
var rust = TSLFn((params) => {
  ...
  var pos = positionGeometry.mul(
    exp(params.scale.add(params.noiseScale.mul(3), 2))
  );
...
var scepterHead = TSLFn((params) => {
  ...
  var k = mx_noise_float(
    vec3(pos.x.div(cosX), pos.y.div(cosY), pos.z.div(cosZ))
  );
  ...
  return mix(
    color1,
    color2,
    remapClamp(max(dx, max(dy, dz)), 55 - 10, 55 + 10)
  );
}, defaults$j);
...
var scream = TSLFn((params) => {
  ...
  return hsl(
    add(HSL.x, params.variety.mul(sin(k.mul(Math.PI))).mul(0.5)),
    HSL.y,
    HSL.z
  );
}, defaults$i);
...
var simplexNoise = TSLFn((params) => {
  ...
  var k = clamp(
    0,
    1,
    mx_noise_float(pos).mul(0.5, exp(params.contrast)).add(0.5, params.balance)
  );
...
var stars = TSLFn((params) => {
  ...
  var dS = select(
    k.greaterThan(0.1),
    params.variation.mul(mx_noise_float(pos)),
    0
  );
...
var voronoiCells = TSLFn((params) => {
  ...
  var randomColor = vec3(n.mul(16.8), n.mul(31.4159), n.mul(27.1828))
    .sin()
    .add(1)
    .div(2);
  ...
  return mix(
    color,
    mix(color, randomColor, params.variation),
    params.variation
  );
}, defaults$b);
...
var watermelon = TSLFn((params) => {
  ...
  var variation = select(
    params.flat,
    params.variation.mul(0.85).add(0.15),
    params.variation
  );
  ...
  var uv = select(
    ...
    a = uv.x
      ...
      .add(mx_noise_float(pos.mul(vec3(1, 0.3, 1))).mul(2));
...
var wood = TSLFn((params) => {
  ...
  var pos = posLocal
    .mul(
      exp(params.scale.sub(3)).mul(
        vec3(reciprocal(params.lengths), 4, reciprocal(params.lengths))
      )
    )
    .add(params.seed)
    .toVar();
  ...
  Loop(10, () => {
    kk.addAssign(
      mul(power, mx_noise_float(posLocal.mul(scale).add(params.seed)))
    );
    sum.addAssign(power);
    scale.mulAssign(1.8);
    power.mulAssign(0.6);
...
var reticularVeins = TSLFn((params) => {
  ...
  var k2 = mx_worley_noise_float(pos.add(100).mul(params.reticulation)).mul(
    params.strength
  );
  var dots = mx_noise_float(pos.mul(100)).mul(
    params.strength,
    params.organelles
  );
...
var turbulentSmoke = TSLFn((params) => {
  ...
  var q = pos.add(
    vec3(
      ...
      mx_fractal_noise_float(
        pos.add(vec3(t.add((2 * Math.PI) / 3).sin(), 1, -1))
      ),
      mx_fractal_noise_float(
        pos.add(vec3(1, -1, t.add((4 * Math.PI) / 3).sin()))
      )
    )
  );
  ...
  var k = mx_worley_noise_float(pos.add(p.div(2)))
    .pow(4)
    .mul(4)
    .oneMinus();
...
export {
  circleDecor as circL,
  caustics as causeL,
  neonLights as neonL,
  isolayers as isoL,
  translator as transL,
